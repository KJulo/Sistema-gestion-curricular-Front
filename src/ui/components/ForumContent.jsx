...
const ForumContent = ({content}) => {
  ...
  function onClickEdit () {setIsEditOpen(true) }
  function onClickCalendar () {setIsCalendarOpen(true) }
  ...
  return (
    <>
      ...
      <Modal title="Editar" open={isEditOpen} onOk={handdleClose} onCancel={handdleClose}>
        <Input.Group>
          <Input defaultValue={content.titulo} size="large" placeholder="Titulo." prefix={<RightOutlined />} id="input"/>
          <TextArea defaultValue={content.cuerpo} rows={6} placeholder="Contenido." id="textArea"/>
        </Input.Group>
      </Modal>

      <Modal title="Añadir tiempo" open={isCalendarOpen} onOk={handdleClose} onCancel={handdleClose}>
